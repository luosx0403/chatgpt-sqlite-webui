    ...
    <div className="modal-backdrop" role="presentation" onClick={onClose}>
      <section className="modal settings-modal" role="dialog" aria-modal="true" aria-labelledby="settings-title" onClick={(event) => event.stopPropagation()}>
        ...
        <div className="settings-grid">
          ...
          <label>{t("messageLayout")}
            <select value={settings.messageLayout} onChange={(event) => update("messageLayout", event.target.value as Settings["messageLayout"])}>
              <option value="chat">{t("chatLayout")}</option>
              <option value="classic">{t("classicLayout")}</option>
            </select>
          </label>
          <label>{t("fontSize")}
