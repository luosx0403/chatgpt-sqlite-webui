...
export type MessageLayout = "chat" | "classic";
...
export function loadSettings(): Settings {
  try {
    ...
    return applyUrlLayoutOverride(raw ? clampSettings(JSON.parse(raw)) : DEFAULT_SETTINGS);
  } catch {
    return applyUrlLayoutOverride(DEFAULT_SETTINGS);
  }
}
...
export function applySettings(settings: Settings) {
  ...
  root.dataset.messageLayout = settings.messageLayout;
...
function applyUrlLayoutOverride(settings: Settings): Settings {
  const search = typeof window === "undefined" ? "" : window.location.search;
  const params = new URLSearchParams(search);
  const layout = params.get("messageLayout") || params.get("layout");
  if (layout !== "chat" && layout !== "classic") return settings;
  return { ...settings, messageLayout: layout };
}
