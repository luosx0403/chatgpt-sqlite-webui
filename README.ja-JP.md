...
exact message search は canonical BLOB を 64 KiB overlap chunk で incremental read し、verified result を connection-local TEMP artifact に保存して count/page/snippet/span/anchor で再利用します。通常の 1 row 上限は decoded 32 MiB characters と UTF-8 32 MiB、trusted local opt-in は 100 MiB characters、raw-only fallback は別に 1 MiB/800,000 characters で、request には独立した aggregate/candidate budget があります。bounded request は confirmed hit と明示的 partial/pending diagnostics を返し、scan を続行できる場合は query、DB identity、generation に bind した signed continuation を返します。`count_total=false` は exact total を主張しません。late hit は row revision に結び付く UTF-8 byte anchor を持ち、reader は数 MiB の page を再生せず直接 seek します。
