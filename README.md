...
The canonical database schema is versioned with `PRAGMA user_version` (current version 4). Version 3 made canonical TEXT identities explicitly `NOT NULL`; version 4 adds durable, field-scoped address and graph revisions so fresh read connections cannot reuse stale compatibility/effective-current decisions. Migration installs the rows and managed triggers in the same write-lock transaction and invalidates optional indexes when required. Read-only commands and Web requests never run migration DDL; older compatible databases return `database_migration_required`. Create and verify an external backup, then run `python chatgpt_archive.py migrate --db archive/chatgpt_archive.db`.
...
## Round 9 resource, ownership, and recovery contracts

Managed FTS, optional Web-index, staging, metadata, generation, and shadow objects are destructively changed only after exact type/table/SQL/fingerprint ownership validation. Collisions are refused with `core_fts_name_collision`, `optional_index_name_collision`, or `staging_name_collision`; user objects are never treated as disposable because their names resemble managed objects. Optional Web index format 4 uses the per-build lease described above. Large placeholder classification streams the complete canonical value, so a prefix longer than 256 characters cannot hide a recoverable placeholder; normalized and derived expansion is bounded before a row is added to a batch.

Long-display cursors bind to a durable per-row revision stored in `archive_generations` under internal `display:<rowid>` keys. Managed insert/update triggers advance that revision for every display-affecting write, including direct external SQLite writers that do not refresh `content_hash`; unrelated rows do not invalidate the cursor. Existing version-4 databases missing these managed triggers are gated as migration-required and the explicit writer migration backfills revisions. Deleted-row revision tombstones are retained to prevent rowid reuse from reviving an old cursor.

Exact message search streams canonical BLOBs in 64 KiB chunks with overlap. The normal per-row ceilings remain 32 MiB of decoded characters and 32 MiB of UTF-8 input, trusted local opt-in through `CHATGPT_ARCHIVE_SEARCH_EXACT_VERIFY_CHARS` can raise the character ceiling to 100 MiB, raw-only fallback has separate 1 MiB/800,000-character ceilings, and one request has independent 128 MiB byte/character aggregate ceilings. An over-limit search returns complete HTTP 413 diagnostics: this version does not return a partial page or promise a continuation token. Initial hit navigation requests an exact total, while later pages may show loaded hits separately. Late matches include a revision-bound character anchor; the reader seeks directly with incremental BLOB access rather than replaying megabytes of pages.

One imported conversation element has independent 32 MiB UTF-8, 32 MiB decoded-character, 250,000-scalar, and 5,000-mapping-node limits. `conversation_node_limit_exceeded` skips that element without retaining its content. The 100,000-node reader, effective-current, and export ceilings exist only to support compatible legacy or externally written databases; they are not an import promise. One ZIP read session is reused across all selected shards, while directory discovery is incremental and budgeted. Empty `parent` is legacy root/missing-parent compatibility. Legacy ID readiness checks every address and graph field for length and unsafe Unicode, and durable field-scoped revisions invalidate cached decisions without polling `PRAGMA data_version` on ordinary reads.

Bulk project imports temporarily replace exact project-owned generation triggers inside the same write-lock transaction and advance each dirty field domain once before restoring and validating those triggers. Rollback or process failure restores the old trigger/data state; external writers continue to use normal per-statement triggers. Finite effective-current scopes are staged and compared exactly in bounded SQLite TEMP batches. Archive export spools its plan and nodes to temporary SQLite and keyset-streams conversations/nodes; it does not retain an archive-wide Python node graph.

With `--delete-input-on-success`, the user's original pathname remains present until the canonical commit succeeds. The post-commit delete protocol durably writes and fsyncs an identity-bound recovery journal before the rename; interruption leaves a token that can be inspected and explicitly restored with `python chatgpt_archive.py recover-delete-input --directory <dir> --token <token>`. Recovery never overwrites a replacement. Secure delete is refused on Windows and on platforms lacking the required descriptor-relative no-follow identity operations. The Web Python constraints pin resolved versions but remain a cross-platform unhashed supply-chain residual; use a trusted package index.
...
Standalone JSON, directory members, and ZIP members use the same single-pass top-level-array framer and one import transaction. Each element is scanned once, decoded once, and independently capped at 32 MiB of UTF-8 input, 32 MiB decoded characters, 256 nesting levels, 250,000 scalar values, and 5,000 mapping nodes. Iterative legacy-raw sanitizing also caps traversal at 100,000 nodes, raw previews at 80,000 bytes, and a complete sanitized API payload at 4 MiB. Source discovery counts every ZIP central-directory entry and every directory entry against a 100,000-member limit. Exactly one file-leading UTF-8 BOM is removed; U+FEFF inside a JSON string is preserved. Canonical IDs are limited to 512 characters and never truncated. Query-based `/api/by-id/*` endpoints are the primary Web addressing contract and accept bounded legacy IDs up to 16 Ki characters, so slash and URL punctuation remain unambiguous; longer legacy IDs make readiness fail with `database_data_incompatible`.

File identity is checked through descriptor-bound stat/hash/read operations. Directory child identity is rechecked at open time. The explicit delete protocol and recovery command are described above; changed targets and replacements are retained. Migration accepts only exact known predecessors. Every managed or optional object name is accepted only with its exact owner fingerprint, otherwise the operation stops before DDL.
...
Default message pages return one reader-budget-bounded `display_text` and truncation/total-exactness metadata, not duplicate `content_text`/`render_text` aliases. Long display text uses a target-row revision-bound opaque cursor over SQLite incremental BLOB reads; unrelated row updates do not stale it. Numeric offset remains capped at 1,048,576 characters, after which the cursor is required. Raw preview uses one NUL-safe bounded BLOB query and reports byte size plus exact/incomplete metadata. Exact search and optional Web index format 4 follow the complete Round 9 contracts above.
...
Request-validation responses contain at most 16 safe items with only allowlisted `location`, `field`, and stable public `code`; they never echo submitted bodies, path/query values, or framework validation types. Raw APIs distinguish exact UTF-8 byte counts from character counts. Message-search candidate verification, late-position snippets, enrichment, and serialized responses are bounded. Imported-size candidates are verified exactly by incremental BLOB reads; a larger legacy candidate is refused with a stable 413 instead of a false-exact result. Web-index byte accounting covers bytes actually read, normalized, and bound to FTS. Release payloads are hashed, written, and verified in streaming chunks rather than whole-file buffers.
