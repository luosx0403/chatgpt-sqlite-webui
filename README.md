...
Import failures use stable preflight, source-scan, JSON-decode, top-level-contract, and transaction stages. Rollback summaries separate `attempted_*` work from zero `committed_*` counts. Failed-run persistence uses a fresh connection and reports any secondary persistence failure. Pre-job temporary cleanup failures preserve the primary HTTP code and add `cleanup_warning` plus a safe `cleanup_error_type`; no temporary path is returned. A successful canonical commit is not described as unimported if later verify, stats, optional Web-index, or cleanup work fails. Import job lookup accepts only lowercase 32-character hexadecimal IDs; malformed IDs return `invalid_job_id` and valid unknown IDs return `job_not_found`.
...
Legacy canonical text that is empty or a legacy placeholder may recover bounded readable text from a valid raw text message. The shared resolver is capped and is used consistently by the reader, message/conversation search, highlights, copy, CLI Markdown/TXT, and Web export. Invalid, oversized, or genuinely non-text raw payloads remain placeholders. Filter-only and exclude-only searches can filter conversations, but only a positive message-text term creates message hits, reader highlights, or hit navigation.
