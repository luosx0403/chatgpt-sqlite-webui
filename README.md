...
## Round 10 resource, ownership, and recovery contracts

Managed FTS, optional Web-index, staging, metadata, generation, and shadow objects are destructively changed only after exact type/table/SQL/fingerprint ownership validation. Collisions are refused with `core_fts_name_collision`, `optional_index_name_collision`, or `staging_name_collision`; user objects are never treated as disposable because their names resemble managed objects. Optional Web index format 5 uses the per-build lease described above. Large placeholder classification streams the complete canonical value, so a prefix longer than 256 characters cannot hide a recoverable placeholder; normalized and derived expansion is bounded before a row is added to a batch.

Long-display cursors bind to a durable revision stored directly on the target message row. Managed insert/update triggers advance that revision for every display-affecting write, including direct external SQLite writers that do not refresh `content_hash`; unrelated rows do not invalidate the cursor. Existing pre-version-5 databases are gated as migration-required and the explicit writer migration backfills revisions. The cursor also binds a stable row identity digest, so rowid reuse cannot revive an old cursor.

Exact message search and positive-body conversation search stream canonical BLOBs in 64 KiB chunks with overlap and store verified results in one connection-local TEMP artifact reused by counting, paging, conversation aggregation/enrichment, snippets, spans, and anchors. The normal per-row ceilings remain 32 MiB of decoded characters and 32 MiB of UTF-8 input, trusted local opt-in through `CHATGPT_ARCHIVE_SEARCH_EXACT_VERIFY_CHARS` can raise the character ceiling to 100 MiB, and one request has independent aggregate and candidate budgets. Raw-only fallback starts at 1 MiB/800,000 characters and a signed continuation can retry the same unresolved row at 5 MiB, 20 MiB, then the effective hard per-row ceiling; a hard-limit row remains pending without an unusable continuation. A bounded request returns confirmed hits plus explicit partial/pending diagnostics and a continuation bound to the query, database identity and generations when scanning can advance. Message pages always expose `total_exact`: it is false for a partial segment, while a completed scan (including the terminal continuation page) can prove the cumulative total even with `count_total=false`. Late matches include a revision-bound UTF-8 byte anchor; hit navigation passes that opaque byte cursor to incremental BLOB reads, while the compatibility character-anchor route may still scan the requested prefix once.

One imported conversation element has independent 32 MiB UTF-8, 32 MiB decoded-character, 1,000,000-lexical-scalar, and 5,000-mapping-node limits. The separate legacy/API sanitizer remains capped at 250,000 scalar values. `conversation_node_limit_exceeded` skips that element without retaining its content. The 100,000-node reader, effective-current, and export ceilings exist only to support compatible legacy or externally written databases; they are not an import promise. One ZIP read session is reused across all selected shards, while directory discovery is incremental and budgeted. Empty `parent` is legacy root/missing-parent compatibility. Legacy ID readiness checks every address and graph field for length and unsafe Unicode, and durable field-scoped revisions invalidate cached decisions without polling `PRAGMA data_version` on ordinary reads.
