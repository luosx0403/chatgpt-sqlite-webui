...
`web-index` scans and normalizes messages, normalizes titles, builds message/title trigram indexes when supported, writes generation metadata, and commits in explicit observable stages. Every data stage uses bounded keyset batches plus byte budgets and resolves each message once. The rebuild stays in one atomic SQLite transaction: readers keep seeing the previous current optional index until commit, while generation/metadata failure, SQLite interruption, disk error, or cancellation rolls back all replacement objects. This holds `BEGIN IMMEDIATE` and one writer slot for the complete build and may use temporary disk. Web import-job JSON exposes stage and processed/total; while that post-import stage is running, the React UI and `POST /api/import/jobs/{job_id}/web-index/cancel` can request cancellation. This is not an independent index-build job. Oversized rows are recorded and canonical-verifier scanned, preserving recall.
...
Standalone JSON, directory members, and ZIP members use the same single-pass top-level-array framer and one import transaction. Each element is scanned once, decoded once, and capped at 32 MiB of UTF-8 input as well as 32 Mi decoded characters, 256 nesting levels, and 100,000 scalar values. Iterative legacy-raw sanitizing also caps traversal at 100,000 nodes, raw previews at 80,000 bytes, and a complete sanitized API payload at 4 MiB. Source discovery counts every ZIP central-directory entry and every directory entry against a 100,000-member limit. Exactly one file-leading UTF-8 BOM is removed; U+FEFF inside a JSON string is preserved. Canonical IDs are limited to 512 characters and never truncated. Query-based `/api/by-id/*` endpoints are the primary Web addressing contract and accept bounded legacy IDs up to 16 Ki characters, so slash and URL punctuation remain unambiguous; longer legacy IDs make readiness fail with `database_data_incompatible`.

File identity is checked through descriptor-bound stat/hash/read operations. Directory child identity is rechecked at open time, and `--delete-input-on-success` uses an atomic staged rename plus a final identity barrier; a changed target is restored or retained rather than deleting a replacement. Migration accepts only explicit predecessor versions, inventories dependent custom views/triggers/indexes before DDL, and refuses unsafe rebuilds with `database_custom_objects_require_manual_migration`; current-version managed drift remains explicitly repairable.
...
Default message pages return one reader-budget-bounded `display_text` and truncation/total-exactness metadata, not duplicate `content_text`/`render_text` aliases. Long display text uses a revision-bound opaque cursor over SQLite incremental BLOB reads; numeric offset remains a compatibility scan capped at 1,048,576 characters, after which the cursor is required. Raw preview uses one NUL-safe bounded BLOB query and reports byte size plus exact/incomplete metadata. Visible NUL and isolated-surrogate recovery is consistently U+FFFD while raw JSON stays safely escaped. Optional Web index format 3 enforces input, normalized, and derived byte budgets; oversized rows are recorded and canonical-verifier scanned so building the index cannot reduce recall. The atomic build holds `BEGIN IMMEDIATE` for the full build, so other writers may wait; Web cancellation is available only for the index stage of a running import job and preserves the previously published optional index.
...
Request-validation responses contain at most 16 safe items with only allowlisted `location`, `field`, and stable public `code`; they never echo submitted bodies, path/query values, or framework validation types. Raw APIs distinguish exact UTF-8 byte counts from character counts. Message-search verification, snippets, enrichment, and response size are bounded and disclose partial oversized-input diagnostics; SQLite rowid BLOB prefixes avoid materializing unbounded canonical text. Web-index byte accounting uses the bytes actually read and normalized. Release payloads are hashed, written, and verified in streaming chunks rather than whole-file buffers.
