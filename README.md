...
Exact message search streams canonical BLOBs in 64 KiB chunks with overlap and stores verified results in one connection-local TEMP artifact reused by counting, paging, snippets, spans, and anchors. The normal per-row ceilings remain 32 MiB of decoded characters and 32 MiB of UTF-8 input, trusted local opt-in through `CHATGPT_ARCHIVE_SEARCH_EXACT_VERIFY_CHARS` can raise the character ceiling to 100 MiB, raw-only fallback has separate 1 MiB/800,000-character ceilings, and one request has independent aggregate and candidate budgets. A bounded request returns confirmed hits plus explicit partial/pending diagnostics; when more candidates can be scanned it supplies a signed continuation bound to the query, database identity and generations. `count_total=false` never claims an exact total. Late matches include a revision-bound UTF-8 byte anchor; the reader seeks directly with incremental BLOB access rather than replaying megabytes of pages.
