...
The canonical database schema is versioned with `PRAGMA user_version` (current version 2). Read-only commands and Web requests never run migration DDL. Before upgrading an older database, create and verify an external backup, then run `python chatgpt_archive.py migrate --db archive/chatgpt_archive.db`; import may upgrade a migratable database as part of its writer transaction. Health and API gates return `database_migration_required` until the migration is complete. FTS5 `message_fts` and Web normalized/trigram tables are optional, rebuildable search capabilities rather than canonical data.
