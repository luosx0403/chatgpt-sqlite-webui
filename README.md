...
The Python 3.12 constraints file is a portable resolved-version profile and remains unhashed across platforms. `requirements-web-py312-macos-arm64.lock` is a wheel-artifact hash lock only for CPython 3.12 on macOS arm64. Installing it with `pip --require-hashes --only-binary=:all:` verifies downloaded wheel bytes; `python tools/verify_web_hash_lock.py` checks only lock structure, target, versions, and hash syntax. Other Python/OS targets must use the portable profile from a trusted index until they have their own generated wheel lock.
...
Reader copy and export actions follow the visible reader contract. `Copy current path conversation` uses a bounded complete-text stream for the current reader path, respects Show internal messages, and ignores search filters. `Copy visible` submits only already loaded visible message IDs to a bounded server stream; all selected rows come from one SQLite read snapshot. Download links use the same current path and Show internal setting. Raw message access is a bounded larger raw preview; truncated responses render `raw_text` as plain text.
...
`/api/schema` reports the effective upload policy and multipart-body limit. Every upload requires a valid `Content-Length`. Before the first body receive, the server checks that length against the multipart parser's temporary filesystem; after parsing it separately checks the project upload/database filesystems, which may be different. Allow space for parser spool, the server-owned ZIP, database growth, WAL/TEMP, and index work. ZIP checks run before import, but JSON decoding, SQLite writes, and `web-index` still consume resources proportional to decoded data. ZIP64 structures are supported and tested with a small forced-ZIP64 member; normal acceptance does not claim a physical archive above 4 GiB.
...
The canonical database schema uses `PRAGMA user_version` (current version 6). Older databases are read-only gated with `database_migration_required`. Migration plans capacity per predecessor step: table rewrites/backfills reserve rewrite and journal space, while the v5 metadata/query-generation step does not assume a full node rewrite. It recomputes authoritative size, rows, sidecars, planned steps, and free space under `BEGIN IMMEDIATE` before mutation. Compatibility scans are exact, batched, cancellable, and report content-free progress. Cancellation, interruption, ENOSPC, or SQLite failure rolls back. A current clean database is a true no-op. Read-only paths never migrate; create and verify an external backup, then run `python chatgpt_archive.py migrate --db archive/chatgpt_archive.db`.
...
Loopback Web access accepts only loopback or explicitly configured hosts. Non-loopback binds require `CHATGPT_ARCHIVE_ALLOWED_HOSTS` (or CLI `--allowed-hosts`); `*` is rejected. Trusted proxy handling uses one strict edge configured with `CHATGPT_ARCHIVE_TRUSTED_PROXIES` or `--trusted-proxies`. Every request validates Host, remote writes require a same-origin `Origin`, and malformed or duplicate Fetch Metadata is rejected. `Sec-Fetch-Site: cross-site` is also rejected for API reads except shallow `/api/health`; static UI, direct navigation (`none`), and clients without Fetch Metadata remain compatible.

Upload checks known body length on the parser spool filesystem before receive, then checks the server-owned ZIP and database filesystems after parsing. Import, migration, and optional Web-index rebuild also run capacity checks with emergency reserves. These are estimates, not guarantees; ENOSPC returns a stable `*_disk_space_insufficient` code, transactions roll back, and a failed private Web-index build preserves the published index.

Standalone JSON, directory members, and ZIP members share a bounded hybrid top-level-array parser and one import transaction. Complete ordinary elements normally succeed in one C-decoder call; uncertain or spanning elements may first take one bounded failed fast probe, then use the persistent lexical framer without repeated prefix decoding. Every element must fit the documented byte, character, token, mapping, array, depth, integer, heap, 5,000-node, and batch limits. Source discovery also bounds central-directory bytes, metadata, paths, and member count. Canonical IDs are limited to 512 characters; query-based `/api/by-id/*` accepts bounded legacy IDs up to 16 Ki characters.
