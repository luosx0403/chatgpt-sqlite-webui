...
Standalone JSON, directory members, and ZIP members use the same single-pass top-level-array framer and one import transaction. Each element is scanned once, decoded once, and capped at 128 MiB of UTF-8 input as well as 128 Mi decoded characters. Exactly one file-leading UTF-8 BOM is removed; U+FEFF inside a JSON string is preserved. Canonical IDs are limited to 512 characters and never truncated. Query-based `/api/by-id/*` endpoints are the primary Web addressing contract and accept bounded legacy IDs up to 16 Ki characters, so slash and URL punctuation remain unambiguous.
