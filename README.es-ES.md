...
El JSON independiente, los miembros de directorio y los miembros ZIP usan el mismo decodificador acotado, elemento por elemento, para una matriz de nivel superior y una sola transacción de importación. Solo se acepta un BOM UTF-8 inicial; se rechazan BOM repetidos o interiores, UTF-16/32, codificaciones mixtas y UTF-8 inválido. Los ID de conversación y del grafo tienen el mismo límite de 512 caracteres que todos los parámetros Web; un ID excesivo omite ese elemento con una advertencia sin contenido y nunca se trunca. La lectura ZIP distingue miembros cifrados, ausentes, cambiados, con CRC fallido y otros fallos de lectura.
