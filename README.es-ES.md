...
`web-index` ejecuta etapas explícitas y observables para escanear/normalizar mensajes, normalizar títulos, crear trigram de mensajes/títulos cuando están disponibles, escribir generation metadata y hacer commit. Cada etapa usa lotes keyset y presupuestos de bytes acotados, y resuelve cada mensaje una sola vez. La reconstrucción vive en una única transacción SQLite atómica: los readers siguen viendo el índice opcional current anterior hasta el commit, y un fallo de generation/metadata, interrupción SQLite, error de disco o cancelación revierte todos los objetos de reemplazo. Mantiene `BEGIN IMMEDIATE` y un writer slot durante toda la construcción y puede usar disco temporal. El JSON del Web import job expone etapa y processed/total; durante la etapa de índice de un import job, la UI React o `POST /api/import/jobs/{job_id}/web-index/cancel` pueden solicitar cancelación. No es un job independiente de construcción. Las filas que exceden el presupuesto se registran y se verifican contra el texto canónico, por lo que crear el índice no reduce el recall.
...
La base canónica usa `PRAGMA user_version` (versión actual 3). La versión 3 añade `NOT NULL` explícito a las identidades TEXT; la migración decide y reconstruye dentro de una sola transacción con bloqueo y rechaza identidades NULL sin adivinarlas. Las lecturas bloquean una base compatible antigua con `database_migration_required` hasta migrarla. Crea y verifica un backup externo antes de migrar.
...
El JSON independiente, directorios y ZIP usan el mismo framer de una pasada y una sola transacción; cada elemento se escanea y decodifica una vez, con límites de 32 MiB para UTF-8 y caracteres decodificados, 256 niveles y 100.000 escalares. El sanitizer iterativo de legacy raw limita el recorrido a 100.000 nodos, el raw preview a 80.000 bytes y el payload API saneado completo a 4 MiB. Todas las entradas del directorio central ZIP y del directorio cuentan para el máximo de 100.000 members. Se acepta solo un BOM inicial y se rechazan las demás codificaciones inválidas. Los ID canónicos nuevos se limitan a 512 caracteres; `/api/by-id/*` admite legacy ID hasta 16 Ki caracteres y un ID antiguo mayor deja readiness en `database_data_incompatible`.

La identidad del archivo se valida con stat/hash/read ligados al descriptor y cada hijo de directorio se revisa al abrir. `--delete-input-on-success` usa rename atómico intermedio y una barrera final, sin borrar un reemplazo. Migration solo acepta predecesores explícitos, inventaría view/trigger/index personalizados dependientes antes de DDL y rechaza rebuild inseguro con `database_custom_objects_require_manual_migration`.
...
La validación devuelve como máximo 16 elementos seguros con solo `location`, `field` y `code` público estable de listas permitidas, sin repetir body, path/query ni tipos internos del framework. Raw API separa bytes UTF-8 exactos de caracteres. Verifier, snippets, enrichment y tamaño de respuesta de búsqueda están acotados y declaran diagnósticos parciales para entradas enormes; prefijos BLOB por rowid evitan materializar texto ilimitado y el índice Web mide bytes realmente leídos/normalizados. El release calcula hash, escribe y verifica payloads por chunks.
