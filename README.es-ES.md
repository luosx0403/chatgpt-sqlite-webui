...
El JSON independiente, los miembros de directorio y los miembros ZIP usan el mismo framer de una sola pasada, elemento por elemento, y una sola transacción de importación; cada elemento se escanea y decodifica una vez, con un máximo de 128 MiB de entrada UTF-8 y 128 Mi caracteres decodificados. Se elimina exactamente un BOM UTF-8 inicial; U+FEFF dentro de una cadena JSON se conserva, mientras que se rechazan BOM iniciales repetidos, BOM interiores fuera de cadenas, UTF-16/32, codificaciones mixtas y UTF-8 inválido. Los ID canónicos nuevos se limitan a 512 caracteres y nunca se truncan. El direccionamiento Web principal usa `/api/by-id/*` con parámetros query y acepta ID legacy acotados a 16 Ki caracteres sin ambigüedad; las rutas path antiguas quedan solo para ID route-safe de 512 caracteres. La lectura ZIP distingue miembros cifrados, ausentes, cambiados, con CRC fallido y otros fallos de lectura.
