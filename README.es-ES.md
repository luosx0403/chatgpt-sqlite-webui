...
La búsqueda exacta de mensajes y la búsqueda de conversaciones con términos positivos de cuerpo leen BLOB canonical en chunks solapados de 64 KiB y guardan un artefacto TEMP verificado reutilizado por count/page/agregación/enrichment/snippet/span/anchor. Los límites normales por fila son 32 MiB de caracteres y 32 MiB UTF-8; el opt-in local confiable admite 100 MiB y cada solicitud tiene presupuestos agregados y de candidatos independientes. El fallback raw-only empieza en 1 MiB/800.000 caracteres; un continuation firmado puede reintentar la misma fila pending con 5 MiB, 20 MiB y después el límite duro efectivo. Una fila por encima del límite duro queda pending sin continuation imposible de avanzar. Los hits confirmados se conservan y solo se ofrece continuation ligado a query, identidad DB y generations cuando el scan puede avanzar. Las páginas de mensajes siempre incluyen `total_exact`: es false en un segmento partial, mientras que un scan terminado (incluida la última página continuation) puede probar el total acumulado incluso con `count_total=false`. La navegación de un hit tardío pasa su byte cursor UTF-8 ligado a revisión al BLOB incremental; la ruta compatible de character anchor todavía puede recorrer una vez el prefijo solicitado.
...
La carga, la importación canónica y la reconstrucción opcional del índice Web comprueban antes la capacidad del sistema de archivos y reservan 256 MiB de emergencia. Son estimaciones conservadoras, no garantías: cuotas, escritores simultáneos, WAL, páginas temporales y la amplificación real de SQLite aún pueden agotar el espacio. Las comprobaciones en ejecución y ENOSPC devuelven `upload_disk_space_insufficient`, `import_disk_space_insufficient` o `web_index_disk_space_insufficient`; se limpia una carga parcial, se revierte la importación canónica y el índice Web publicado anterior sigue siendo legible.
