...
La carga, importación canónica, migration de schema e índice Web comprueban capacidad y reservan 256 MiB. ENOSPC devuelve `upload_disk_space_insufficient`, `import_disk_space_insufficient`, `migration_disk_space_insufficient` o `web_index_disk_space_insufficient`; import/migration revierten y el index previo sigue legible.

JSON, directorios y ZIP usan el mismo framer single-pass y una transacción. El framer recorre una vez hasta el límite del elemento, llama una vez al decoder C y aplica el perfil conjunto anterior. `/api/by-id/*` admite legacy ID hasta 16 Ki caracteres; el continuation fijo no incluye esos IDs.
