...
from .resource_contract import import_batch_resource_profile
from .scanner import (
...
def create_api_router(
    ...
    def schema_docs():
        ...
        return {
            ...
            "versions": {
                ...
                "search_continuation_format_version": SEARCH_CONTINUATION_VERSION,
            },
            "pagination": {
                "conversation_page": ["items", "total", "limit", "offset", "has_more", "next_offset", "order_exact", "scan_complete", "provisional_order"],
                "message_page": ["items", "total", "limit", "offset", "has_more", "next_offset"],
                "message_search_page": ["items", "total", "total_exact", "limit", "offset", "has_more", "next_offset", "order_exact", "scan_complete", "provisional_order"],
                ...
                "order_exact": "true means this page belongs to the final globally ordered result; false with provisional_order=true means candidate-cursor segments are still provisional",
                "scan_complete": "true means the bounded candidate scan reached its terminal state",
                "continuation_offset": "when a continuation token is present or supplied, next_offset is null because numeric offsets are not equivalent to candidate-cursor continuation",
            },
            "id_addressing": {
            ...
            "conversations": {
                ...
                "response": ["total", "items", "has_more", "next_offset", "order_exact", "scan_complete", "provisional_order", "selected_in_results", "selected_item", "node_count", "current_path_nodes", "current_node_exists", "current_collection_source", "current_path_fallback_to_all", "effective_path", "cycle_detected", "missing_parent", "cross_conversation_parent", "partial_chain", "raw_flag_leaf_count", "selected_chain_cycle_detected", "raw_flag_cycle_detected", "selected_chain_missing_parent", "raw_flag_missing_parent", "selected_chain_cross_conversation_parent", "raw_flag_cross_conversation_parent"],
            ...
            "web_index_build": {
                ...
                "writer_lock": {
                    ...
                    "busy": "writer_process_lock_busy",
                },
                "ownership_errors": ["core_fts_name_collision", "optional_index_name_collision", "staging_name_collision", "web_index_build_in_progress", "web_index_process_lock_failed", "writer_process_lock_busy", "writer_process_lock_unsupported", "database_identity_changed"],
            },
            "search": {
                ...
                "count_total": "boolean; false disables the eager count. A completed candidate scan may still prove total_exact=true; an incomplete scan returns a known lower bound with total_exact=false.",
                "message_resource_contract": {
                    ...
                    "continuation": "a fixed-size signed opaque server-instance session ID refers to bounded in-memory state that binds database identity, schema, Web-index format and stable-address identity, resolver/normalization versions, all five durable generations (message, title, graph, address, query), the complete query/filter/path/scope/match/order/page contract, cumulative state, and budget version. Query generation covers source_file and conversation/node create/update times in addition to insert/delete membership. Restart, secret rotation, cross-worker routing without affinity, expiry, any query-visible external writer change, index rebuild, metadata change, or database replacement returns invalid_search_continuation or search_continuation_stale; no conversation/node ID is embedded in the token.",
                ...
                "diagnostics": {
                    "fields": [
                        ...
                        "partial",
                        "partial_reason",
                        "configured_candidate_scan_char_limit",
                        "configured_verified_char_limit_per_candidate",
                        "configured_verified_byte_limit_per_candidate",
                        "verified_chars_per_candidate",
                        "verified_bytes_per_candidate",
                        "max_observed_verified_chars_per_candidate",
                        "max_observed_verified_bytes_per_candidate",
                        "request_verified_chars",
                        "request_verified_bytes",
                        "candidate_sql_rows",
                        "candidates_seen",
                        "candidates_verified",
            ...
            "jobs": {
                ...
                "fields": ["status", "stage", "outcome", "completion_outcome", "canonical_import_outcome", "canonical_commit_succeeded", "error_code", "error_type", "cleanup_warning", "cleanup_warnings", "summary", "verify", "stats", "web_index", "web_index_cancel_requested", "web_index_cancelled", "stage_timings"],
                "completion_outcomes": ["queued", "running", "success", "success_with_warnings", "partial_success", "failed_before_commit", "failed_after_canonical_commit", "cleanup_warning", "cancelled"],
                "canonical_import_outcomes": ["queued", "running", "success", "success_with_warnings", "partial_success", "failed_before_commit"],
                "summary_contract": {
                    "fields": ["committed_conversations", "committed_nodes", "skipped_invalid_elements", "warnings", "warnings_by_type", "json_resource_profile"],
                    "privacy": "aggregate counts and stable warning codes only; no title, message, raw JSON, conversation/node ID, filename, path, OS message, or exception text",
                },
                "stage_timings": "content-free wall seconds accumulated separately for upload, canonical import (including lock handoff and post-commit summary persistence), verify, stats, optional Web-index build, and cleanup. A job is not terminal until upload cleanup and writer-lock release complete.",
            ...
            "import_contract": {
                ...
                "joint_resource_contract": "A conversation must fit every listed byte, character, token, mapping-entry, array-item, nesting, integer, decoded-heap, node, and import-batch materialization limit. Crossing any dimension skips that element with a structured warning while later valid elements continue.",
            ...
            "request_validation": {
                ...
                "privacy": "never returns submitted input, request bodies, raw query/path values, Pydantic reprs, paths, OS messages, or tracebacks. Identifier parameters reject Unicode Cc controls and isolated surrogates; output filenames and Content-Disposition replace them safely while retaining ordinary Unicode and noncharacters.",
            },
            "stable_error_codes": [
    ...
    def conversations(
        ...
        _validate_identifier_parameter(selected_id)
        parsed = parse_query(
            q,
            path_default=path,
            role=role,
    ...
    def conversation_messages(
        ...
        _validate_identifier_parameter(conversation_id)
        _validate_identifier_parameter(around_node_id)
    ...
    def conversation_message_display(
        ...
        _validate_identifier_parameter(conversation_id)
        _validate_identifier_parameter(node_id)
        try:
...
def _content_disposition(filename: str) -> str:
    ...
    clean_name, _changed = unicode_scalar_text(clean_name, replace_invalid=True)
    clean_name = "".join(
        ch for ch in (clean_name or "") if unicodedata.category(ch) != "Cc"
    ).strip().rstrip(" .")
...
def _validate_identifier_parameter(value: str | None) -> None:
    if value is None:
        return
    if not identifier_text_is_safe(
        value,
        limit=MAX_LEGACY_ID_PARAM_LENGTH,
        allow_empty=False,
    ):
        raise HTTPException(status_code=400, detail="invalid_identifier_token")
